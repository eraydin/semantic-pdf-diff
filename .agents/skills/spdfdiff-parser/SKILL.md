...
- Controlled classic xref tables, `/Type /XRef` streams, `/Type /ObjStm`
  ...
  `/StructTreeRoot` structure-tree, `/RoleMap`, plus `/ParentTree` entry
  parsing are implemented parser capabilities. Supported stream filters may
  appear as ordered filter chains, and paired `/DecodeParms` metadata must be preserved.
